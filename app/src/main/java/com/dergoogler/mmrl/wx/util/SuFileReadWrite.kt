 ...
 */
fun SuFile.readNBytes(): ByteArray = inputStream().use { input ->
    var offset = 0
...
fun SuFile.writeNBytes(array: ByteArray): Unit = SuFileOutputStream(this).use { it.write(array) }
 ...
 */
@Throws(IOException::class)
fun SuFile.readNText(charset: Charset = Charsets.UTF_8): String =
 ...
 */
@Throws(IOException::class)
fun SuFile.writeNText(text: String, charset: Charset = Charsets.UTF_8): Unit =
...
fun SuFile.forEachBlock(action: (buffer: ByteArray, bytesRead: Int) -> Unit): Unit =
    forEachBlock(DEFAULT_BLOCK_SIZE, action)
