(function() {
    ...
    const modId = currentScript.getAttribute('data-mod-id');
    const inputStream = currentScript.getAttribute('data-input-stream');

    // Configuration
    var BASE_MODULE_PATH = '/data/adb/modules';
    var CURRENT_MODULE_ID = modId;
    ...
    function loadFromFileSystem(filePath) {
        try {
            var input = window[inputStream].open(filePath);
            var result = [];
